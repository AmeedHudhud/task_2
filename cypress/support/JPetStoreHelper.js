...
export const verify1 = (product, isExist = true) => {
    if (isExist) {
...
export const checkTotalCost = (locator, row) => {
    cy.get(`input[name="${locator}"]`).then(($input) => { //quantity
        const value = parseFloat($input.val());
        cy.get(LOCATORS.tableRows).eq(row).children().eq(5).invoke('text').then((text) => { //price
            const price = parseFloat(text.replace(/\$/g, ""));
            const total = (value * price).toFixed(2);
            cy.get(LOCATORS.tableRows).eq(row).children().eq(6).invoke('text').should('eq', '$' + total);
        });
    });
}
export const checkSubTotal = () => {
    ...
    cy.get(LOCATORS.tableRows).then((value) => {
        ...
        value.slice(1, -1).each((index, row) => {
            cy.wrap(row).then((x) => {
                cy.get(row).children().eq(5).invoke('text').then((text) => {
...
export const verify = (data, row, quantity = false) => {
    ...
    if (quantity == false) {
        cy.get('tr').eq(row).invoke('text').then((value) => {
...
export const checkButtonFunctionality1 = (locator, type, button, page = "main") => {
    if (button == "Return to Main Menu") {
        clickButton(LOCATORS.cart, false);
        clickButton(button);
        cy.get("#Main").should("exist");
